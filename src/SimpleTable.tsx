...
export const SimpleTable = <T extends {}>({
  ...
  const [columnSettings, setColumnSettings] = useColumnSettingsStorage(id, columns);
  ...
  const x = useMemo(() => interColumns.reduce((width, col) => width + (col.width || 0), 0), [
    interColumns,
  ]);
  ...
  const handleDownload = useCallback((): void => {
    ...
    const downloadColumns = interColumns.filter((column): boolean => !!column.dataIndex);
    ...
    dataSource.forEach((record): void => {
      ...
      downloadColumns.forEach((column): void => {
        const dataIndex: string =
          column.dataIndex instanceof Array ? column.dataIndex.join('.') : `${column.dataIndex}`;

        row[typeof column.title === 'string' ? column.title : dataIndex] = get(record, dataIndex);
      });

      data.push(row);
    });

    const blob = new Blob([new Uint8Array([0xef, 0xbb, 0xbf]), Papa.unparse(data)], {
